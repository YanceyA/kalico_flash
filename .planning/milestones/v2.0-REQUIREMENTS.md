# Requirements Archive: v2.0 Public Release
...
- [x] **SAFE-01**: Before stopping Klipper, query Moonraker for print status
- [x] **SAFE-02**: Block flash if `print_stats.state` is `printing` or `paused`
- [x] **SAFE-03**: Error message shows print filename and progress percentage
- [x] **SAFE-04**: Allow flash if printer is idle, complete, cancelled, or error state
- [x] **SAFE-05**: If Moonraker unreachable, warn and prompt user to confirm continue
- [x] **SAFE-06**: Moonraker URL configurable in settings (default: localhost:7125) — OUT OF SCOPE (per CONTEXT.md: keep it simple)
...
- [x] **SKIP-01**: `--skip-menuconfig` / `-s` flag skips TUI if cached config exists
- [x] **SKIP-02**: Error with helpful message if no cached config
- [x] **SKIP-03**: MCU validation still runs (prevents config/device mismatch)
- [x] **SKIP-04**: Flag works with `--device KEY`
- [x] **SKIP-05**: TUI menu offers skip option as y/n prompt before flash

### Error Messages

- [x] **ERR-01**: All error paths include contextual recovery guidance
- [x] **ERR-02**: Messages include specific diagnostic commands to copy/paste
- [x] **ERR-03**: No generic "operation failed" without explanation
- [x] **ERR-04**: Recovery steps are numbered and actionable
- [x] **ERR-05**: Messages fit on standard 80-column terminal
- [x] **ERR-06**: Error categories: Build, Device not found, Moonraker, MCU mismatch, Service control, Flash

### Version Detection

- [x] **VER-01**: Show host Klipper version before flash (from git describe)
- [x] **VER-02**: Show MCU firmware version before flash (from Moonraker API)
- [x] **VER-03**: Indicate whether update is needed (commit count comparison)
- [x] **VER-04**: Default prompt answer reflects recommendation — N/A (informational only, no prompt)
- [x] **VER-05**: Gracefully handle unreachable Moonraker (skip check, warn user)
- [x] **VER-06**: Gracefully handle unresponsive MCU (skip check, proceed)
- [x] **VER-07**: Works with multiple MCUs (checks the one being flashed)
...
- [x] **EXCL-01**: Registry supports `flashable: false` flag on devices
- [x] **EXCL-02**: Non-flashable devices shown in `--list-devices` with indicator
- [x] **EXCL-03**: Non-flashable devices excluded from flash device selection
- [x] **EXCL-04**: `--add-device` wizard asks if device is flashable
- [x] **EXCL-05**: Beacon probe can be registered as non-flashable
...
**Shipped:** 55 of 55 v2.0 requirements

**Adjusted:**
- SAFE-06: Moonraker URL configurable — marked OUT OF SCOPE (hardcoded localhost:7125 per CONTEXT.md simplicity decision)
- VER-04: Default prompt answer — marked N/A (version check is informational only, no prompt)
- VERIFY-02: Timeout changed from 15s to 30s for RP2040 compatibility
