...
**Target features:**
- Remove all argparse setup and CLI flags (--device, --add-device, --list-devices, --remove-device, etc.)
- `kflash` entry point launches directly into interactive TUI (no flags accepted)
- Device keys auto-generated by slugifying user-provided display name (e.g., "Octopus Pro v1.1" → `octopus-pro-v1-1`)
- Collision handling for duplicate slugs (append numeric suffix)
- Registry lookup by name or key internally
- Remove device key from user-facing surfaces (add-device wizard, config screen, all prompts)
- Device config screen: remove key edit option, hide internal key entirely
