...
**Add module-level import after existing imports:**
```python
...
Also ensure SETTINGS in `kflash/screen.py` has `"min"` and `"max"` keys for numeric settings. Check current SETTINGS definition — if min/max are missing, add them:
