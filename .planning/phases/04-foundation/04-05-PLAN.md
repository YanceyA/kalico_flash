...
must_haves:
  truths:
    - "ServiceError messages include recovery steps before raising"
  ...
  artifacts:
    - path: "kalico-flash/service.py"
      provides: "ServiceError raised with format_error() message"
...
<task type="auto">
  <name>Task 1: Update service.py to use format_error() for ServiceError</name>
  <files>kalico-flash/service.py</files>
  <action>
Import format_error from errors module and use it when raising ServiceError.
...
Also update _start_klipper() warning messages to use format_error():
The _start_klipper function prints warnings rather than raising (to not mask errors).
Update the print statements to use format_error for consistency:
...
Note: _start_klipper does not raise - it prints and continues (by design, to not mask
the original error). Keep this behavior but use format_error for the message.
  </action>
  <verify>
Run: `python -c "import service"` (no syntax errors)
Run: `grep "format_error" kalico-flash/service.py` (shows 3+ uses)
  </verify>
  <done>service.py raises ServiceError with formatted messages including recovery steps</done>
</task>
...
In validate_mcu() for missing .config, replace:
```python
raise ConfigError(
    f"No .config file in klipper directory: {self.klipper_dir}"
)
...
In validate_mcu() for missing CONFIG_MCU, replace:
```python
raise ConfigError(
...
code in flash.py (Plan 04) will wrap these in error_with_recovery(). No changes needed
...
<success_criteria>
- service.py uses format_error() for ServiceError (2 raise sites) and warnings (3 print sites)
