...
1. **device_not_registered** (line 97-100): Update recovery to include BOTH suggestions:
   - "Use Add Device from the main menu to register it"
   - "Or check registered devices with the main menu device list"
   - Keep "Check device key spelling" or similar
   This gives the user two paths: register a new device OR verify what is already registered.

2. **device_not_connected** (line 108): Change "press A to re-register" to "use Add Device from the main menu to re-register".

3. **mcu_mismatch** (line 117): Change "Or update device registration if MCU changed" to "Or use Config Device from the main menu to update MCU". Line 3 about grep is fine (diagnostic command).

4. **printer_busy** (line 198): Change "Then re-run flash command" to "Then use Flash Device from the main menu".

5. **device_excluded** (line 207): Change recovery to direct users to Config Device specifically: "Use Config Device from the main menu to manage device exclusion settings". Do NOT suggest viewing devices — the user needs to change the exclusion, not just see it.
...
Also search the rest of errors.py for any string containing "--", "flag", or "CLI" that might need updating.
  ...
  <verify>Search errors.py for "Press ", "--", "re-run flash command", "re-run" — none should match. Search for "main menu" — should find matches in updated templates. Search for "Config Device" — should match in device_excluded and mcu_mismatch templates. Search for "Add Device" — should match in device_not_registered template.</verify>
  <done>All error recovery messages reference TUI action names. No key shortcuts ("Press F"), no CLI flags, no "re-run" commands. device_not_registered suggests both Add Device and viewing registered devices. device_excluded directs to Config Device. DOC-03 satisfied.</done>
</task>
...
5. `grep -n "Config Device" kflash/errors.py` — matches in device_excluded and mcu_mismatch
6. `grep -n "Add Device" kflash/errors.py` — matches in device_not_registered
</verification>
...
- device_not_registered suggests both adding and viewing devices
- device_excluded directs to Config Device for exclusion management
- DOC-03 satisfied
</success_criteria>
