...
must_haves:
  truths:
    ...
    - "User can identify which MCU is being flashed in version table"
  requirement_notes:
    - "VER-04 (default prompt answer) is NOT APPLICABLE - version check is informational only, no prompt per 05-CONTEXT.md: 'Version comparison is informational only — never blocks flash'"
    - "SAFE-06 (Moonraker URL configurable) is OUT OF SCOPE per 05-CONTEXT.md decision"
  artifacts:
...
- Requirements addressed: SAFE-01 through SAFE-05, VER-01, VER-02, VER-03, VER-05, VER-06, VER-07
- VER-04 not applicable (version check is informational only, no prompt)
- SAFE-06 out of scope per CONTEXT.md decision
</success_criteria>
