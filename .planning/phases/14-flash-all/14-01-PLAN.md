...
- For each device, check if cached config exists via `ConfigManager(entry.key, global_config).get_cached_config_path()` — if the returned path exists on disk.
...
- Get host version and MCU versions. If Moonraker unavailable (returns None), skip version check with a warning and proceed with all devices.
...
- For each device in the flash list:
  ...
  - Load cached config: `ConfigManager(entry.key, global_config).load_cached_config(klipper_dir)` — read ConfigManager to find the exact method signature.
