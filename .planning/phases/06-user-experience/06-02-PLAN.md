...
4. **Remove device handler (numbered list approach for better UX):**
   ```python
   def _handle_remove_device(registry, out) -> None:
       """Remove a registered device via numbered selection."""
       data = registry.load()
       devices = list(data.devices.items())  # [(key, DeviceEntry), ...]
       ...
       # Get choice using existing helper
       valid_choices = [str(i) for i in range(1, len(devices) + 1)]
       choice = _get_menu_choice(valid_choices, out, max_attempts=3)

       if choice is None:
           out.warn("Cancelled")
           return

       # Extract device key from selection
       selected_index = int(choice) - 1
       device_key = devices[selected_index][0]
...
5. Menu option 4 (Remove) -> shows numbered device list -> user picks number -> confirms removal -> returns to menu
