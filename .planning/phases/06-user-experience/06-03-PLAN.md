...
must_haves:
  truths:
    ...
    - "Klipper service restarts even if verification fails"
  artifacts:
...
2. **After flash_result.success check, add verification:**
   Replace current success path (around line 583-587):
   ...
   if flash_result.success:
       ...
       if verified:
           ...
           # Verification failed but flash may have worked
           out.warn(f"Device verification failed: {error_reason}")
           if device_path:
               # Device appeared but wrong state
           ...
           # Note: Klipper already restarted by context manager
           return 1
   ```

3. **Per VERIFY-06:** "Klipper still restarted even if verification fails" - this is already handled by the `with klipper_service_stopped():` context manager. Service restarts on ANY exit from the block.

4. **Per VERIFY-04:** "Success message shows device path" - included in the verified=True branch.

Note: The verification happens AFTER Klipper restarts (context manager exit), so the device should be available for Klipper to connect to.

Wait - actually need to rethink: verification should happen BEFORE context manager exit, because:
- Flash happens inside context manager (Klipper stopped)
- Device reboot happens after flash
- Device should reappear before Klipper tries to connect

So move verification inside the `with` block, after flash_device() but before context manager exits. Update structure:
...
try:
    with klipper_service_stopped():
        ...
        if flash_result.success:
            ...
            out.phase("Verify", "Waiting for device to reappear...")
            verified, device_path, error_reason = wait_for_device(entry.serial_pattern)
        else:
            verified = False
            device_path = None
...
Then handle verified/not-verified after the context manager (after Klipper restart).
  </action>
  <verify>
...
Manual test on Pi: Flash device, observe "Verifying...." with dots, see success message with path
  ...
  <done>cmd_flash() calls wait_for_device() after flash, shows device path on success, recovery steps on failure</done>
</task>
