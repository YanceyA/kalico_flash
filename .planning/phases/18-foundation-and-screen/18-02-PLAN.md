...
<task type="auto">
  ...
  Note: "key" is a virtual setting — DeviceEntry doesn't have a "key" field (it's the dict key in registry). render_device_config_screen must accept `device_key: str` as a separate parameter.

  Define `render_device_config_screen(device_key: str, device_entry: DeviceEntry) -> str`:
  ...
  - Panel 2 — "settings" (numbered, editable):
    - For each setting in DEVICE_SETTINGS, numbered starting at 1:
      - For "key" type=text: display `device_key` (the parameter)
      - For "name" type=text: display `device_entry.name`
  ...
  - render_device_config_screen accepts device_key and device_entry params
  - Two render_panel calls: "device identity" and "settings"
  - Numbered settings follow same formatting as render_config_screen
  - "key" setting displays device_key parameter, not getattr
  </verify>
